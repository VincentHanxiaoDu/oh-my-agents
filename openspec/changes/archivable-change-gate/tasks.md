...
- [x] Write `bin/check-openspec-archivable.sh`, project-owned, running `openspec validate --strict` over the changes in scope
...
- [x] Write `bin/check-openspec-archivable.sh --self-test` covering every arm, and watch each go red under a mutation
- [x] Write `tests/openspec-archivable.test.sh` against a fixture built from this repository's real change
- [x] Wire both into `make ci` via an `archivable` target, extending #11's Makefile in its own shape
- [x] Make every non-pass name the gate and say it is not a failing test, since it has no commit status of its own
