...
```
./bin/watch.sh queue qa 60      # instead of ./.workflow/bin/watch-queue.sh qa 60
./bin/watch.sh prs   qa 60      # instead of ./.workflow/bin/watch-prs.sh   qa 60
./bin/watch.sh status queue qa  # who, if anyone, is watching — starts nothing
```
...
the quota symptom: two `qa` sessions watching one queue are both told to act on the same pull
request, which the `[qa]` dedup marker does not make safe.
