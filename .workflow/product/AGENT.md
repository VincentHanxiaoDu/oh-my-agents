...
newcomer would get wrong, and anything the product role needs that is true of this project and not of
every project.
...
```
./bin/watch.sh queue product 60      # instead of ./.workflow/bin/watch-queue.sh product 60
./bin/watch.sh prs   product 60      # instead of ./.workflow/bin/watch-prs.sh   product 60
./bin/watch.sh status queue product  # who, if anyone, is watching — starts nothing
```
...
the quota symptom: two `product` sessions watching one queue are both told to act on the same pull
request, which the `[product]` dedup marker does not make safe.
