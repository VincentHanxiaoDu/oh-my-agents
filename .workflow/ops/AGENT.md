...
```
./bin/watch.sh queue ops 60      # instead of ./.workflow/bin/watch-queue.sh ops 60
./bin/watch.sh prs   ops 60      # instead of ./.workflow/bin/watch-prs.sh   ops 60
./bin/watch.sh status queue ops  # who, if anyone, is watching — starts nothing
```
...
the quota symptom: two `ops` sessions watching one queue are both told to act on the same pull
request, which the `[ops]` dedup marker does not make safe.
